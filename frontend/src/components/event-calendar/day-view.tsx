      ...
      <div className="grid flex-1 grid-cols-[3rem_1fr] overflow-hidden border-border/70 border-t sm:grid-cols-[4rem_1fr]">
        <div>
          {hours.map((hour, index) => (
            ...
            >
              {index > 0 && (
                <span className="-top-3 absolute left-0 flex h-6 w-16 max-w-full items-center justify-end bg-slate-50 dark:bg-background px-1 pe-2 text-[10px] text-muted-foreground sm:pe-4 sm:text-xs font-medium">
                  {format(hour, "HH:mm")}
                </span>
              )}
            </div>
